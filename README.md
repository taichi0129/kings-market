...
# 開発の感想
- 初めて一から考えて開発したアプリケーション。自ら作りたいと思ったものを考えての開発は非常に楽しかったです。  
また実装したい機能を創意工夫して実装出来たときは感極まる部分もありました。  
これからも思いのこもったアプリを作成していきたいと思います。

# 苦労した点
- 今まで実装したことがない、知らない機能を実装するために自ら調べて実装するのには非常に苦労しました。  
ただ、自ら調べて実装することで検索力には自信が付き、更にはコードをコピペするのではなく、  
１行１行理解しながらコーディングしていくことで、MVCの理解度も上げることができました。

- 開発前に綿密にデータベースは考えたものの、製作途中にテーブルやカラムなどを変更することが多々ありました。
アプリの動作や使い勝手、ビューのイメージなど、もう少し具体的にアプリケーションの内容をイメージしておけば、  
よりスムーズに開発が出来たと感じています。
